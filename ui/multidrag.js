 ...
 *
 * http://api.jqueryui.com/multidrag/
 */
(function( factory ) {
...
$.widget("ui.multidrag", $.ui.mouse, {
	...
	_create: function() {
		...
		if (this.options.addClasses){
			this.element.addClass("ui-multidrag");
		}
		...
		// cache multidragged children based on filter
		this.refresh = function() {
			multidrag_set = $(that.options.filter, that.element[0]);
			multidrag_set.addClass("ui-multidraggable");
			multidrag_set.each(function() {
				var $this = $(this),
					pos = $this.offset();
				$.data(this, "multidrag-item", {
					element: this,
					$element: $this,
					left: pos.left,
					top: pos.top,
					...
					startselected: false,
					selected: $this.hasClass("ui-multidrag-selected"),
					selecting: $this.hasClass("ui-multidrag-selecting"),
					unselecting: $this.hasClass("ui-multidrag-unselecting")
				});
			});
		};
		this.refresh();
		...
		this.select_helper = $("<div class='ui-multidrag-helper'></div>");
	},
	...
	_destroy: function() {
		...
		this.multidrag_set
			.removeClass("ui-multidraggable ui-multidragging")
			.removeData("multidrag-item");
		this.element
			.removeClass("ui-multidrag ui-multidrag-disabled");
		this._mouseDestroy();
	},
	...
	_mouseDrag: function(event, noPropagation) {
		...
		if ( this.drag_originator == "container") {

			if (x1 > x2) { tmp = x2; x2 = x1; x1 = tmp; }
			if (y1 > y2) { tmp = y2; y2 = y1; y1 = tmp; }
			this.select_helper.css({ left: x1, top: y1, width: x2 - x1, height: y2 - y1 });

			this.multidrag_set.each(function() {
				var multidragged = $.data(this, "multidrag-item"),
					hit = false;

				//prevent helper from being selected if appendTo: multidrag
				if (!multidragged || multidragged.element === that.element[0]) {
					return;
				}

				if (options.tolerance === "touch") {
					hit = ( !(multidragged.left > x2 || multidragged.right < x1 || multidragged.top > y2 || multidragged.bottom < y1) );
				} else if (options.tolerance === "fit") {
					hit = (multidragged.left > x1 && multidragged.right < x2 && multidragged.top > y1 && multidragged.bottom < y2);
				}

				if (hit) {
					// SELECT
					if (multidragged.selected) {
						multidragged.$element.removeClass("ui-multidrag-selected");
						multidragged.selected = false;
					}
					if (multidragged.unselecting) {
						multidragged.$element.removeClass("ui-multidrag-unselecting");
						multidragged.unselecting = false;
					}
					if (!multidragged.selecting) {
						multidragged.$element.addClass("ui-multidrag-selecting");
						multidragged.selecting = true;
						// multidrag SELECTING callback
						that._trigger("selecting", event, {
							selecting: multidragged.element
						});
					}
					...
					// UNSELECT
					if (multidragged.selecting) {
						if ((event.metaKey || event.ctrlKey) && multidragged.startselected) {
							multidragged.$element.removeClass("ui-multidrag-selecting");
							multidragged.selecting = false;
							multidragged.$element.addClass("ui-multidrag-selected");
							multidragged.selected = true;
						} else {
							multidragged.$element.removeClass("ui-multidrag-selecting");
							multidragged.selecting = false;
							if (multidragged.startselected) {
								multidragged.$element.addClass("ui-multidrag-unselecting");
								multidragged.unselecting = true;
							}
							// multidrag UNSELECTING callback
	...
	_mouseStop: function(event) {
		...
		if (this.drag_origininator == "container" || !this.dropped) {
			$(".ui-multidrag-unselecting", this.element[0]).each(function() {
				...
				multidragged.$element.removeClass("ui-multidrag-unselecting");
				multidragged.unselecting = false;
				multidragged.startselected = false;
				that._trigger("unselected", event, {
					unselected: multidragged.element
				});
			});
			$(".ui-multidrag-selecting", this.element[0]).each(function() {
				var multidragged = $.data(this, "multidrag-item");
				multidragged.$element.removeClass("ui-multidrag-selecting").addClass("ui-multidrag-selected");
				multidragged.selecting = false;
				multidragged.selected = true;
				multidragged.startselected = true;
				that._trigger("selected", event, {
					selected: multidragged.element
				});
			});
		}
		if ( this.drag_originator == "multidrag") {
		...
		this.select_helper.remove();

		return false;
	},

	// Okay, this WHOLE section needs to be assessed
...
$.ui.plugin.add("multidrag", "zIndex", {
	start: function( event, ui, instance ) {
		var t = $( ui.drag_helper ),
			o = instance.options;
	...
	stop: function( event, ui, instance ) {
		...
		if (o._zIndex) {
			$(ui.drag_helper).css("zIndex", o._zIndex);
		}
	}
});
